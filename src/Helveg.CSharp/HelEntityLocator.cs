...
public class HelEntityLocator
{
    private readonly HelSolutionCS solution;

    public HelEntityLocator(HelSolutionCS solution)
    {
        this.solution = solution;
    }

    public HelProjectCS? FindProject(string name)
    {
        return solution.Projects.SingleOrDefault(p => p.Name == name);
    }

    public HelPackageCS? FindPackage(string name)
    {
        return solution.Packages.SingleOrDefault(p => p.Name == name);
    }

    public HelAssemblyCS? FindAssembly(HelAssemblyIdCS id)
    {
    ...
    public HelModuleCS? FindModule(string name)
    {
        return solution.Projects.Select(p => p.Assembly)
            .Concat(solution.Packages.SelectMany(p => p.Assemblies))
    ...
    public HelNamespaceCS? FindNamespace(string moduleName, string? namespaceName)
    {
    ...
    public HelTypeCS? FindType(string moduleName, string? namespaceName, string typeName, int arity)
    {
