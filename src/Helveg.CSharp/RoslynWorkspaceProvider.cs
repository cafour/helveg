...
public class RoslynWorkspaceProvider : IHelWorkspaceCSProvider
{
    ...
    /// </summary>
    private class SymbolTranscriber
    {
        private readonly EntityTokenSymbolVisitor visitor;
        private readonly Project project;

        public SymbolTranscriber(
            Project project,
        ...
        {
            var found = visitor.Tokens.TryGetValue(symbol, out var token);
            var reference = new HelAssemblyReferenceCS
            {
                Token = found ? token : GetErrorToken(HelEntityKindCS.Assembly),
        ...
        private HelModuleReferenceCS GetModuleReference(IModuleSymbol symbol)
        {
            ...
            {
                Token = found ? token : GetErrorToken(HelEntityKindCS.Module),
                Hint = symbol.ToDisplayString(),
                IsError = !found
            };
            return reference;
        ...
        private HelNamespaceReferenceCS GetNamespaceReference(INamespaceSymbol symbol)
        {
            ...
            {
                Token = found ? token : GetErrorToken(HelEntityKindCS.Namespace),
                Hint = symbol.ToDisplayString(),
                IsError = !found
            };
            return reference;
        ...
        private HelTypeReferenceCS GetTypeReference(ITypeSymbol symbol)
        {
            ...
            switch (symbol)
            {
                case IArrayTypeSymbol arrayType:
                    var arrayReference = GetTypeReference(compilation.GetSpecialType(SpecialType.System_Array))
