...
export interface ForceAtlas2Options extends Full<ForceAtlas2Settings> {
    autoStopAverageTraction: number
}

export interface ForceAtlas2Metadata {
...
export default function iterate(
    ...
    let l: number; // index into ???
    let r: number; // index into RegionMatrix
    let n: number; // indices into NodeMatrix
    let n1: number;
    let n2: number;
    let rn: number;
    let e: number; // index into EdgeMatrix
    let w: number; // edge weight
    let g: number; // gravity but also not?
    let s: number; // seems to be connected to barnes-hut?

    let order = NodeMatrix.length;
    let size = EdgeMatrix.length;

    let adjustSizes = options.adjustSizes;

    let thetaSquared = options.barnesHutTheta * options.barnesHutTheta;

    let outboundAttCompensation: number = 0;
                ...
                if (RegionMatrix[r + REGION_FIRST_CHILD] >= 0) {
                    ...
                    if (RegionMatrix[r + REGION_NODE] < 0) {
                        ...
                        if (NodeMatrix[RegionMatrix[r + REGION_NODE] + NODE_X] < RegionMatrix[r + REGION_CENTER_X]) {
                            if (
                                NodeMatrix[RegionMatrix[r + REGION_NODE] + NODE_Y] < RegionMatrix[r + REGION_CENTER_Y]
                            ) {
                                // Top Left quarter
                                q = RegionMatrix[r + REGION_FIRST_CHILD];
                            } else {
                ...
                if (RegionMatrix[r + REGION_FIRST_CHILD] >= 0) {
                    ...
                    if ((4 * s * s) / distance < thetaSquared) {
                        ...
                        if (adjustSizes === true) {
                            ...
                            if (distance > 0) {
                                factor =
                                    (coefficient * NodeMatrix[n + NODE_MASS] * RegionMatrix[r + REGION_MASS]) /
                                    distance;
                    ...
                    if (rn >= 0 && rn !== n) {
                        ...
                        if (adjustSizes === true) {
                            ...
                            if (distance > 0) {
                                factor =
                                    (coefficient * NodeMatrix[n + NODE_MASS] * NodeMatrix[rn + NODE_MASS]) / distance;

                                NodeMatrix[n + NODE_DX] += xDist * factor;
                                NodeMatrix[n + NODE_DY] += yDist * factor;
                            } else if (distance < 0) {
                                factor =
                                    (-coefficient * NodeMatrix[n + NODE_MASS] * NodeMatrix[rn + NODE_MASS]) /
                                    Math.sqrt(distance);
